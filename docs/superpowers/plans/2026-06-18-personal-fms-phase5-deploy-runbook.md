...
> **Hard precondition for the whole phase (§12 note, §14.24):** the CloudPanel site-user naming on THIS box must be confirmed before any path is used. Task 5.0 establishes it. Every later task assumes site user `argontechs-fms`, home `/home/money`, docroot `/home/argontechs-fms/htdocs/fms.argontechs.dev` — **substitute the real values from 5.0 if they differ**.
...
- [ ] Confirm no path mixing: `getent passwd | grep -E 'money|htdocs' || true` and `ls -d /home/*/htdocs/* 2>/dev/null`. **Verify:** the SSH user, docroot owner, and crontab owner will be ONE uniform name. Write it into `docs/deploy/RUNBOOK.md` under a `## Confirmed values (2026-06-18)` heading with literal lines `SITE_USER=argontechs-fms`, `SITE_HOME=/home/argontechs-fms`, `DOCROOT=/home/argontechs-fms/htdocs/fms.argontechs.dev`, `NODE_VERSION=22`.
- [ ] Add the DNS A record `money` → VPS public IPv4 at the DNS provider (AAAA too if the box has IPv6). **Verify:** `dig +short fms.argontechs.dev` returns the VPS public IP exactly (run `curl -s ifconfig.me` on the box to get the IP to compare). Do NOT proceed to the cert step (5.8) until this matches — Let's Encrypt HTTP-01 fails otherwise.
- [ ] Commit the runbook doc: `git add docs/deploy/RUNBOOK.md && git commit -m "docs(deploy): record confirmed CloudPanel site-user naming + DNS for fms.argontechs.dev"`.
...
- [ ] Create the committed template `.env.example` with the variable NAMES only and empty/placeholder values (no real secrets):
  ...
  DATABASE_URL=file:/home/argontechs-fms/data/money.sqlite
  VAPID_PUBLIC_KEY=
...
- Produces: Linux site user `argontechs-fms`, docroot `/home/argontechs-fms/htdocs/fms.argontechs.dev`, nginx vhost reverse-proxying `→ 127.0.0.1:3000`.
...
- [ ] As root, create the site (use a strong generated password; capture it once into a password manager, never into the repo or logs):
  ...
  clpctl site:add:nodejs \
    ...
    --nodejsVersion=22 \
    --appPort=3000 \
    --siteUser=argontechs-fms \
    --siteUserPassword="$SITE_PW"
  ```
  Then store `$SITE_PW` in the password manager and `unset SITE_PW`.
- [ ] **Verify** the site user + docroot exist with correct ownership: `ls -ld /home/argontechs-fms/htdocs/fms.argontechs.dev && stat -c '%U:%G' /home/argontechs-fms/htdocs/fms.argontechs.dev`. Expected: directory exists, owned `argontechs-fms:argontechs-fms`.
- [ ] **Verify** the nginx vhost was generated and targets the loopback app port: `grep -Rn '127.0.0.1:3000' /etc/nginx/ 2>/dev/null || clpctl site:list`. Expected: a `proxy_pass http://127.0.0.1:3000;` line (or equivalent CloudPanel vhost config) exists for `fms.argontechs.dev`.
- [ ] Append the confirmed site password location (not the value) and the vhost path to `docs/deploy/RUNBOOK.md`, then commit: `git add docs/deploy/RUNBOOK.md && git commit -m "docs(deploy): record CloudPanel nodejs site creation for fms.argontechs.dev"`.
...
- [ ] SSH as the **site user** (`ssh money@<vps>`), then clone into the docroot (note the trailing `.` to clone into the existing dir):
  ...
  ```
  **Verify:** `git -C /home/argontechs-fms/htdocs/fms.argontechs.dev rev-parse --short HEAD` prints a commit SHA.
...
**Files**
- Create (on box, NEVER committed): `/home/argontechs-fms/htdocs/fms.argontechs.dev/.env`
...
- [ ] Write `/home/argontechs-fms/htdocs/fms.argontechs.dev/.env` with real values (DB path is OUTSIDE the docroot at `/home/argontechs-fms/data`; secrets via `openssl rand`):
  ```bash
  NODE_ENV=production
  NITRO_HOST=127.0.0.1
  NITRO_PORT=3000
...
- [ ] Lock permissions: `chmod 600 /home/argontechs-fms/htdocs/fms.argontechs.dev/.env`. **Verify:** `stat -c '%a %U' /home/argontechs-fms/htdocs/fms.argontechs.dev/.env` → `600 argontechs-fms`.
- [ ] **Verify both VAPID public copies are identical AND the DB path is outside docroot:**
  ```bash
  set -a; . /home/argontechs-fms/htdocs/fms.argontechs.dev/.env; set +a
...
**Files**
- Create (on box): `/home/argontechs-fms/data/` (700), `/home/argontechs-fms/backups/` (700), `/home/argontechs-fms/data/money.sqlite`
...
**Steps**
- [ ] As the site user: `mkdir -p /home/argontechs-fms/data /home/argontechs-fms/backups /home/argontechs-fms/logs && chmod 700 /home/argontechs-fms/data /home/argontechs-fms/backups`. **Verify:** `stat -c '%a' /home/argontechs-fms/data /home/argontechs-fms/backups` → `700` and `700`.
- [ ] Install deps + run migrations + seed against the production DB path (Phase 1 npm scripts read `DATABASE_URL` from `.env`):
  ...
  ```
  **Verify migrate created the DB file:** `ls -l /home/argontechs-fms/data/money.sqlite` → file exists, owned `money`.
- [ ] **Verify the mandated pragmas are live (WAL + FK):**
  ```bash
  sqlite3 /home/argontechs-fms/data/money.sqlite "PRAGMA journal_mode; PRAGMA foreign_keys;"
  ```
...
- [ ] **Verify the v1 tables exist (the 8 shipped tables):**
  ```bash
  sqlite3 /home/argontechs-fms/data/money.sqlite ".tables"
  ```
...
- [ ] **Verify seed sanity against the real figures (§ seed data):**
  ```bash
  sqlite3 /home/argontechs-fms/data/money.sqlite "SELECT count(*) FROM recurring_items;"   # expect 16
  sqlite3 /home/argontechs-fms/data/money.sqlite "SELECT balance_cents FROM debts WHERE name='Credit Card';"  # expect 740076
  sqlite3 /home/argontechs-fms/data/money.sqlite "SELECT payoff_baseline_cents FROM debts WHERE name='Credit Card';"  # expect 740076 (frozen at goal creation, §14.3)
  sqlite3 /home/argontechs-fms/data/money.sqlite "SELECT available_credit_cents FROM accounts WHERE type='card';"  # expect NULL/0 — DERIVED at read, never seeded (§14.2)
  sqlite3 /home/argontechs-fms/data/money.sqlite "SELECT target_amount_cents FROM goals WHERE name='Emergency Fund';"  # expect 100000 (RM1,000 starter, §14.16)
  ```
...
- [ ] **Verify exactly one user row exists and the hash is argon2id (not plaintext/bcrypt):**
  ```bash
  sqlite3 /home/argontechs-fms/data/money.sqlite "SELECT count(*) FROM users;"   # expect 1
  sqlite3 /home/argontechs-fms/data/money.sqlite "SELECT substr(password_hash,1,10) FROM users;"  # expect '$argon2id$'
  ```
  Expected: `1` and `$argon2id$`.
- [ ] **Verify no secret leaked to shell history or logs:** `! grep -RniE 'register_token|password=' ~/.bash_history /home/argontechs-fms/logs/ 2>/dev/null || (echo 'FAIL: secret in history/logs'; false)`. Expected: no output, exit 0. (Since the CLI prompts on TTY, the password is never an argv token.)
...
- [ ] Install the systemd boot unit for the **site user** (run the printed `sudo env PATH=... pm2 startup ...` line as root):
  ```bash
  pm2 startup systemd -u argontechs-fms --hp /home/argontechs-fms   # copy the printed sudo line, run it as root
  pm2 save
  ```
  **Verify the unit exists:** as root, `systemctl status pm2-argontechs-fms --no-pager | head -3` → shows `pm2-argontechs-fms.service` loaded.
- [ ] **Enable linger (the line that actually makes it survive reboot, §12.5/§14.24) — as root:** `loginctl enable-linger argontechs-fms`. **Verify:** `loginctl show-user argontechs-fms --property=Linger` → `Linger=yes`.
- [ ] **Reboot and verify FROM A ROOT SHELL (SSHing as the site user masks a missing linger, §14.24):**
  ...
  pgrep -u argontechs-fms -fa 'PM2|money-fms' && sudo -u argontechs-fms pm2 list
  ```
  Expected: a `money-fms` process owned by `money` is running, and `pm2 list` shows it `online` — **without any interactive site-user login having occurred**. If it is NOT running, linger failed — re-run `loginctl enable-linger argontechs-fms` and reboot again before proceeding.
...
**Interfaces**
- Consumes: the running `money-fms` on `127.0.0.1:3000` (5.7); confirmed DNS (5.0); the Phase-4 login handler that hard-sets the session cookie `httpOnly:true, secure:true, sameSite:'lax', domain:'fms.argontechs.dev'` in code (§14.6 — `setCookie` does NOT auto-add Secure, and `X-Forwarded-Proto` is spoofable from loopback, so the cookie attributes are set in code, not inferred from the proto header).
- Produces: HTTPS at `fms.argontechs.dev` with HTTP→HTTPS redirect, HSTS, dotfile deny, and a verified Secure session cookie.
...
- [ ] Ensure the vhost forwards the proto header + websocket upgrade and denies dotfiles. In the CloudPanel vhost for `fms.argontechs.dev`, confirm the proxy `location` block contains:
  ```nginx
...
- [ ] Issue the cert: CloudPanel → Sites → `fms.argontechs.dev` → SSL/TLS → New Let's Encrypt Certificate → Create and Install (enables auto HTTP→HTTPS redirect + auto-renew). **Verify TLS + redirect:**
  ```bash
  curl -sI http://fms.argontechs.dev/  | grep -i '^location:'   # expect https://fms.argontechs.dev/
  curl -sI https://fms.argontechs.dev/ | head -1                # expect HTTP/2 200 (or 302 to login)
  ```
  Expected: HTTP redirects to `https://`; HTTPS returns a 200/302.
- [ ] **Verify HSTS + dotfile deny + X-Forwarded-Proto reaching the app:**
  ```bash
  curl -sI https://fms.argontechs.dev/ | grep -i 'strict-transport-security'   # expect HSTS header
  curl -sI https://fms.argontechs.dev/.env                                       # expect 403
  ```
...
- [ ] **Verify the session cookie is hard-set Secure (§14.6) — log in and inspect Set-Cookie:**
  ```bash
  curl -si https://fms.argontechs.dev/api/auth/login -X POST \
    -H 'content-type: application/json' \
    -d '{"password":"<owner-password>"}' | grep -i '^set-cookie:'
  ```
  Expected: the `Set-Cookie` line contains `Secure`, `HttpOnly`, `SameSite=Lax`, and `Domain=fms.argontechs.dev` (all four, set in code — NOT inferred from the spoofable proto header).
...
**Files**
- Create (repo, deployed to box): `bin/deploy.sh` → installed at `/home/argontechs-fms/bin/deploy.sh`
...
- [ ] Install it on the box and run a real deploy: `mkdir -p /home/argontechs-fms/bin && cp bin/deploy.sh /home/argontechs-fms/bin/deploy.sh && chmod 700 /home/argontechs-fms/bin/deploy.sh && /home/argontechs-fms/bin/deploy.sh`. **Verify:** the final line prints `Deployed money-fms @ <sha>` and `pm2 jlist` shows `money-fms` `online` with a fresh restart time.
- [ ] **Verify the VAPID public key is actually in the served client bundle (§12.6 — a missing key fails every push silently):** `curl -s https://fms.argontechs.dev/ | grep -o 'vapidPublicKey[^,}]*' | head -1` OR fetch the runtime config: `curl -s https://fms.argontechs.dev/api/push/vapid-public-key 2>/dev/null` (if Phase-3 exposes it) — confirm it returns the same public key as `NUXT_PUBLIC_VAPID_PUBLIC_KEY`. Expected: the non-empty public key string is present in the client payload.
...
**Files**
- Create (repo, deployed to box): `bin/backup-db.sh` → `/home/argontechs-fms/bin/backup-db.sh`
...
- [ ] Write `bin/backup-db.sh` with rclone BEFORE prune, non-fatal upload + alert, off-box retention, chmod 600 (the §14.7 ordering is load-bearing — never prune local before the off-box copy succeeds):
  ...
  #!/usr/bin/env bash
  set -euo pipefail
  DB=/home/argontechs-fms/data/money.sqlite
  DEST=/home/argontechs-fms/backups
...
- [ ] Install and run once for real: `cp bin/backup-db.sh /home/argontechs-fms/bin/ && chmod 700 /home/argontechs-fms/bin/backup-db.sh && set -a; . /home/argontechs-fms/htdocs/fms.argontechs.dev/.env; set +a; /home/argontechs-fms/bin/backup-db.sh`. **Verify the snapshot is valid + landed off-box + perms:**
  ```bash
  ls -l /home/argontechs-fms/backups/money-*.sqlite.gz | tail -1                        # archive exists
  stat -c '%a' "$(ls -t /home/argontechs-fms/backups/money-*.sqlite.gz | head -1)"      # expect 600
  rclone lsf gdrive-money:money-fms-backups/ | tail -1                          # same file on Drive
  zcat "$(ls -t /home/argontechs-fms/backups/money-*.sqlite.gz|head -1)" > /tmp/v.sqlite && sqlite3 /tmp/v.sqlite "PRAGMA integrity_check;" && rm /tmp/v.sqlite
  ```
...
**Files**
- Create (repo, deployed to box): `bin/restore-verify.sh` → `/home/argontechs-fms/bin/restore-verify.sh`
...
- [ ] Write `bin/restore-verify.sh` (restores the newest gz to scratch and asserts integrity + non-empty ledger + a recent transaction; §14.7 mandates integrity_check + row-count + latest-timestamp):
  ...
  ```
- [ ] **Verify lint + a real restore-verify run against the archive from 5.10:** `shellcheck bin/restore-verify.sh` (expect clean), then `cp bin/restore-verify.sh /home/argontechs-fms/bin/ && chmod 700 /home/argontechs-fms/bin/restore-verify.sh && /home/argontechs-fms/bin/restore-verify.sh && tail -1 /home/argontechs-fms/backups/restore-verify.log`. Expected: shellcheck clean; log tail shows `restore-verify OK: ... (integrity ok, recurring=16, users=1, txns=...)`.
- [ ] Install BOTH cron jobs for the site user (backup at 03:15 MYT, restore-verify at 03:45 MYT — both env-sourced for `RCLONE_REMOTE`/`DATABASE_URL`; crontab `TZ` pins MYT):
  ...
  ( echo "TZ=Asia/Kuala_Lumpur"
    echo "15 3 * * * set -a; . /home/argontechs-fms/htdocs/fms.argontechs.dev/.env; set +a; /home/argontechs-fms/bin/backup-db.sh >> /home/argontechs-fms/backups/backup.log 2>&1"
    echo "45 3 * * * /home/argontechs-fms/bin/restore-verify.sh >> /home/argontechs-fms/backups/restore-verify.log 2>&1"
  ) | crontab -u argontechs-fms -
  ```
  **Verify:** `crontab -u argontechs-fms -l` lists `TZ=Asia/Kuala_Lumpur`, the `backup-db.sh` line, and the `restore-verify.sh` line (backup time before restore-verify time).
...
**Files**
- Create (repo, deployed to box): `bin/run-due-watchdog.sh` → `/home/argontechs-fms/bin/run-due-watchdog.sh`
...
- [ ] Install the script + cron line (every 5 minutes, matching the in-process `*/5` cadence so the watchdog catches the same window):
  ```bash
  cp bin/run-due-watchdog.sh /home/argontechs-fms/bin/ && chmod 700 /home/argontechs-fms/bin/run-due-watchdog.sh
  ( crontab -u argontechs-fms -l; echo "*/5 * * * * set -a; . /home/argontechs-fms/htdocs/fms.argontechs.dev/.env; set +a; /home/argontechs-fms/bin/run-due-watchdog.sh >> /home/argontechs-fms/logs/run-due.log 2>&1" ) | crontab -u argontechs-fms -
  ```
  **Verify:** `crontab -u argontechs-fms -l | grep run-due-watchdog` shows the `*/5` line. This watchdog is PERMANENT (§14.10) — do not remove it after confirming croner.
...
**Steps**
- [ ] On the confirmed iPhone, open `https://fms.argontechs.dev` in Safari, **Share → Add to Home Screen**, then open the installed (standalone) app and log in. **Verify standalone:** the app's iOS health signal (§8) shows "running standalone" (not the "Add to Home Screen" banner). iOS web push only works standalone — a Safari tab silently fails.
- [ ] In the standalone app, tap **"Turn on reminders"** (user-gesture-initiated, required by iOS) → accept the permission prompt → the client calls `pushManager.subscribe({userVisibleOnly:true, applicationServerKey:<vapidPublicKey>})` → `POST /api/push/subscribe`. **Verify the subscription row landed (and the endpoint is session-gated, not open):**
  ```bash
  sqlite3 /home/argontechs-fms/data/money.sqlite "SELECT id, substr(endpoint,1,40), user_agent, failed_at FROM push_subscriptions ORDER BY id DESC LIMIT 1;"
  ```
  Expected: a fresh row with a real `endpoint`, an iPhone user-agent, and `failed_at` NULL.
- [ ] **Verify the open-subscribe attack is blocked (§9/§14.12 — /api/push/subscribe is gated):** `curl -s -o /dev/null -w '%{http_code}\n' -X POST https://fms.argontechs.dev/api/push/subscribe -H 'content-type: application/json' -d '{"endpoint":"https://x","keys":{"p256dh":"x","auth":"x"}}'`. Expected: `401` (no session cookie → `requireSession` throws). An unauthenticated subscribe MUST NOT create a row.
- [ ] **Verify a real canary push is DELIVERED end-to-end.** Trigger the "reminders are working" canary (§8) — either the app's post-enable canary, or fire `notify-dispatch` manually via the secret-gated watchdog when a reminder is due: `set -a; . /home/argontechs-fms/htdocs/fms.argontechs.dev/.env; set +a; curl -s -X POST http://127.0.0.1:3000/api/internal/run-due -H "x-internal-secret: $INTERNAL_RUN_DUE_SECRET"`. **Verify:** the canary notification physically appears on the iPhone Home-Screen-installed app AND `pm2 logs money-fms --lines 50 --nostream | grep -iE 'web-push|push send'` shows a successful send (no `404/410`). If the device receives nothing but logs show success, re-check `NUXT_PUBLIC_VAPID_PUBLIC_KEY` equals `VAPID_PUBLIC_KEY` (5.4) — a public/private mismatch is the classic silent failure.
- [ ] **Verify failed-subscription pruning works (defensive):** confirm a `404/410` from `web-push` sets `failed_at` (inspect after any stale endpoint): `sqlite3 /home/argontechs-fms/data/money.sqlite "SELECT count(*) FROM push_subscriptions WHERE failed_at IS NOT NULL;"`. Expected: `0` for the just-created live subscription (it is healthy). Record the end-to-end PASS in `docs/deploy/RUNBOOK.md` and commit: `git add docs/deploy/RUNBOOK.md && git commit -m "docs(deploy): record subscribe() end-to-end push smoke-test PASS on iPhone standalone (§12.6)"`.

---

#### Phase deliverable & how to verify

**Deliverable:** the Personal-FMS PWA live at `https://fms.argontechs.dev` — HTTPS via Let's Encrypt, ONE PM2 fork app (`money-fms`, app + in-process croner) surviving reboot via `loginctl enable-linger`, daily WAL-safe `.backup` snapshots gzipped and pushed off-box to the named `gdrive-money` remote BEFORE local prune, a restore-verify cron proving the backups restore (`integrity_check` + row-count + freshness), a permanent loopback/secret-gated `/api/internal/run-due` OS-cron watchdog behind the in-process scheduler, and a verified end-to-end push subscription on the iPhone.

**Single end-to-end verification pass (run from a ROOT shell after a fresh `sudo reboot`):**
1. `dig +short fms.argontechs.dev` → VPS IP; `curl -sI https://fms.argontechs.dev/ | head -1` → `200/302`; HTTP→HTTPS redirect + HSTS present (5.8).
2. `sudo -u argontechs-fms pm2 jlist | grep -c '"name":"money-fms"'` → `1`, status `online`, `exec_mode:fork`, `instances:1`; `loginctl show-user argontechs-fms -p Linger` → `Linger=yes` (5.7).
3. `sqlite3 /home/argontechs-fms/data/money.sqlite "PRAGMA journal_mode; PRAGMA integrity_check;"` → `wal` / `ok`; `.tables` shows all 9 v1 tables; `recurring_items=16`, `users=1`, card `balance_cents=740076`, `payoff_baseline_cents=740076`, EF target `100000` (5.5/5.6).
4. `crontab -u argontechs-fms -l` → three jobs (`backup-db.sh` 03:15, `restore-verify.sh` 03:45, `run-due-watchdog.sh` `*/5`) under `TZ=Asia/Kuala_Lumpur`; `tail -1 /home/argontechs-fms/backups/restore-verify.log` → `restore-verify OK`; newest `gdrive-money:money-fms-backups/` object matches the local archive (5.10/5.11/5.12).
5. `curl -sI https://fms.argontechs.dev/.env` → `403`; `curl -sI https://fms.argontechs.dev/api/internal/run-due` → `404`; unauthenticated `POST /api/push/subscribe` → `401`; login `Set-Cookie` carries `Secure; HttpOnly; SameSite=Lax; Domain=fms.argontechs.dev` (5.8/5.12/5.14).
...
**Files produced (repo-tracked):** `/Users/brendxn___/Desktop/Personal-FMS/.gitignore`, `.env.example`, `bin/deploy.sh`, `bin/backup-db.sh`, `bin/restore-verify.sh`, `bin/run-due-watchdog.sh`, `docs/deploy/RUNBOOK.md`. **Files produced (box-only, never committed):** `/home/argontechs-fms/htdocs/fms.argontechs.dev/.env` (chmod 600), `/home/argontechs-fms/.config/rclone/rclone.conf` (chmod 600), `/home/argontechs-fms/data/money.sqlite`, `ecosystem.config.cjs` PM2 dump, the site-user crontab, the `pm2-money` systemd unit.