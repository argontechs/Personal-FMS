...
> **Hard precondition for the whole phase (§12 note, §14.24):** the CloudPanel site-user naming on THIS box must be confirmed before any path is used. Task 5.0 establishes it. Every later task assumes site user `money`, home `/home/money`, docroot `/home/fms/htdocs/fms.argontechs.dev` — **substitute the real values from 5.0 if they differ**.
...
- Produces: Linux site user `money`, docroot `/home/fms/htdocs/fms.argontechs.dev`, nginx vhost reverse-proxying `→ 127.0.0.1:3000`.
...
- [ ] **Verify** the site user + docroot exist with correct ownership: `ls -ld /home/fms/htdocs/fms.argontechs.dev && stat -c '%U:%G' /home/fms/htdocs/fms.argontechs.dev`. Expected: directory exists, owned `money:money`.
...
- [ ] SSH as the **site user** (`ssh money@<vps>`), then clone into the docroot (note the trailing `.` to clone into the existing dir):
  ...
  ```
  **Verify:** `git -C /home/fms/htdocs/fms.argontechs.dev rev-parse --short HEAD` prints a commit SHA.
...
**Files**
- Create (on box, NEVER committed): `/home/fms/htdocs/fms.argontechs.dev/.env`
...
- [ ] Write `/home/fms/htdocs/fms.argontechs.dev/.env` with real values (DB path is OUTSIDE the docroot at `/home/fms/data`; secrets via `openssl rand`):
  ```bash
  NODE_ENV=production
  NITRO_HOST=127.0.0.1
  NITRO_PORT=3000
...
- [ ] Lock permissions: `chmod 600 /home/fms/htdocs/fms.argontechs.dev/.env`. **Verify:** `stat -c '%a %U' /home/fms/htdocs/fms.argontechs.dev/.env` → `600 money`.
- [ ] **Verify both VAPID public copies are identical AND the DB path is outside docroot:**
  ```bash
  set -a; . /home/fms/htdocs/fms.argontechs.dev/.env; set +a
...
**Files**
- Create (on box): `/home/fms/data/` (700), `/home/fms/backups/` (700), `/home/fms/data/money.sqlite`
...
**Steps**
- [ ] As the site user: `mkdir -p /home/fms/data /home/fms/backups /home/fms/logs && chmod 700 /home/fms/data /home/fms/backups`. **Verify:** `stat -c '%a' /home/fms/data /home/fms/backups` → `700` and `700`.
- [ ] Install deps + run migrations + seed against the production DB path (Phase 1 npm scripts read `DATABASE_URL` from `.env`):
  ...
  ```
  **Verify migrate created the DB file:** `ls -l /home/fms/data/money.sqlite` → file exists, owned `money`.
- [ ] **Verify the mandated pragmas are live (WAL + FK):**
  ```bash
  sqlite3 /home/fms/data/money.sqlite "PRAGMA journal_mode; PRAGMA foreign_keys;"
  ```
...
- [ ] **Verify the v1 tables exist (the 8 shipped tables):**
  ```bash
  sqlite3 /home/fms/data/money.sqlite ".tables"
  ```
...
- [ ] **Verify seed sanity against the real figures (§ seed data):**
  ```bash
  sqlite3 /home/fms/data/money.sqlite "SELECT count(*) FROM recurring_items;"   # expect 16
  sqlite3 /home/fms/data/money.sqlite "SELECT balance_cents FROM debts WHERE name='Credit Card';"  # expect 740076
  sqlite3 /home/fms/data/money.sqlite "SELECT payoff_baseline_cents FROM debts WHERE name='Credit Card';"  # expect 740076 (frozen at goal creation, §14.3)
  sqlite3 /home/fms/data/money.sqlite "SELECT available_credit_cents FROM accounts WHERE type='card';"  # expect NULL/0 — DERIVED at read, never seeded (§14.2)
  sqlite3 /home/fms/data/money.sqlite "SELECT target_amount_cents FROM goals WHERE name='Emergency Fund';"  # expect 100000 (RM1,000 starter, §14.16)
  ```
...
- [ ] **Verify exactly one user row exists and the hash is argon2id (not plaintext/bcrypt):**
  ```bash
  sqlite3 /home/fms/data/money.sqlite "SELECT count(*) FROM users;"   # expect 1
  sqlite3 /home/fms/data/money.sqlite "SELECT substr(password_hash,1,10) FROM users;"  # expect '$argon2id$'
  ```
  Expected: `1` and `$argon2id$`.
- [ ] **Verify no secret leaked to shell history or logs:** `! grep -RniE 'register_token|password=' ~/.bash_history /home/fms/logs/ 2>/dev/null || (echo 'FAIL: secret in history/logs'; false)`. Expected: no output, exit 0. (Since the CLI prompts on TTY, the password is never an argv token.)
...
**Files**
- Create (repo, deployed to box): `bin/deploy.sh` → installed at `/home/fms/bin/deploy.sh`
...
- [ ] Install it on the box and run a real deploy: `mkdir -p /home/fms/bin && cp bin/deploy.sh /home/fms/bin/deploy.sh && chmod 700 /home/fms/bin/deploy.sh && /home/fms/bin/deploy.sh`. **Verify:** the final line prints `Deployed money-fms @ <sha>` and `pm2 jlist` shows `money-fms` `online` with a fresh restart time.
...
**Files**
- Create (repo, deployed to box): `bin/backup-db.sh` → `/home/fms/bin/backup-db.sh`
...
- [ ] Write `bin/backup-db.sh` with rclone BEFORE prune, non-fatal upload + alert, off-box retention, chmod 600 (the §14.7 ordering is load-bearing — never prune local before the off-box copy succeeds):
  ...
  #!/usr/bin/env bash
  set -euo pipefail
  DB=/home/fms/data/money.sqlite
  DEST=/home/fms/backups
...
- [ ] Install and run once for real: `cp bin/backup-db.sh /home/fms/bin/ && chmod 700 /home/fms/bin/backup-db.sh && set -a; . /home/fms/htdocs/fms.argontechs.dev/.env; set +a; /home/fms/bin/backup-db.sh`. **Verify the snapshot is valid + landed off-box + perms:**
  ```bash
  ls -l /home/fms/backups/money-*.sqlite.gz | tail -1                        # archive exists
  stat -c '%a' "$(ls -t /home/fms/backups/money-*.sqlite.gz | head -1)"      # expect 600
  rclone lsf gdrive-money:money-fms-backups/ | tail -1                          # same file on Drive
  zcat "$(ls -t /home/fms/backups/money-*.sqlite.gz|head -1)" > /tmp/v.sqlite && sqlite3 /tmp/v.sqlite "PRAGMA integrity_check;" && rm /tmp/v.sqlite
  ```
...
**Files**
- Create (repo, deployed to box): `bin/restore-verify.sh` → `/home/fms/bin/restore-verify.sh`
...
- [ ] Write `bin/restore-verify.sh` (restores the newest gz to scratch and asserts integrity + non-empty ledger + a recent transaction; §14.7 mandates integrity_check + row-count + latest-timestamp):
  ...
  ```
- [ ] **Verify lint + a real restore-verify run against the archive from 5.10:** `shellcheck bin/restore-verify.sh` (expect clean), then `cp bin/restore-verify.sh /home/fms/bin/ && chmod 700 /home/fms/bin/restore-verify.sh && /home/fms/bin/restore-verify.sh && tail -1 /home/fms/backups/restore-verify.log`. Expected: shellcheck clean; log tail shows `restore-verify OK: ... (integrity ok, recurring=16, users=1, txns=...)`.
- [ ] Install BOTH cron jobs for the site user (backup at 03:15 MYT, restore-verify at 03:45 MYT — both env-sourced for `RCLONE_REMOTE`/`DATABASE_URL`; crontab `TZ` pins MYT):
  ...
  ( echo "TZ=Asia/Kuala_Lumpur"
    echo "15 3 * * * set -a; . /home/fms/htdocs/fms.argontechs.dev/.env; set +a; /home/fms/bin/backup-db.sh >> /home/fms/backups/backup.log 2>&1"
    echo "45 3 * * * /home/fms/bin/restore-verify.sh >> /home/fms/backups/restore-verify.log 2>&1"
  ) | crontab -u money -
  ```
...
**Files**
- Create (repo, deployed to box): `bin/run-due-watchdog.sh` → `/home/fms/bin/run-due-watchdog.sh`
...
- [ ] Install the script + cron line (every 5 minutes, matching the in-process `*/5` cadence so the watchdog catches the same window):
  ```bash
  cp bin/run-due-watchdog.sh /home/fms/bin/ && chmod 700 /home/fms/bin/run-due-watchdog.sh
  ( crontab -u money -l; echo "*/5 * * * * set -a; . /home/fms/htdocs/fms.argontechs.dev/.env; set +a; /home/fms/bin/run-due-watchdog.sh >> /home/fms/logs/run-due.log 2>&1" ) | crontab -u money -
  ```
...
- [ ] In the standalone app, tap **"Turn on reminders"** (user-gesture-initiated, required by iOS) → accept the permission prompt → the client calls `pushManager.subscribe({userVisibleOnly:true, applicationServerKey:<vapidPublicKey>})` → `POST /api/push/subscribe`. **Verify the subscription row landed (and the endpoint is session-gated, not open):**
  ```bash
  sqlite3 /home/fms/data/money.sqlite "SELECT id, substr(endpoint,1,40), user_agent, failed_at FROM push_subscriptions ORDER BY id DESC LIMIT 1;"
  ```
...
- [ ] **Verify a real canary push is DELIVERED end-to-end.** Trigger the "reminders are working" canary (§8) — either the app's post-enable canary, or fire `notify-dispatch` manually via the secret-gated watchdog when a reminder is due: `set -a; . /home/fms/htdocs/fms.argontechs.dev/.env; set +a; curl -s -X POST http://127.0.0.1:3000/api/internal/run-due -H "x-internal-secret: $INTERNAL_RUN_DUE_SECRET"`. **Verify:** the canary notification physically appears on the iPhone Home-Screen-installed app AND `pm2 logs money-fms --lines 50 --nostream | grep -iE 'web-push|push send'` shows a successful send (no `404/410`). If the device receives nothing but logs show success, re-check `NUXT_PUBLIC_VAPID_PUBLIC_KEY` equals `VAPID_PUBLIC_KEY` (5.4) — a public/private mismatch is the classic silent failure.
- [ ] **Verify failed-subscription pruning works (defensive):** confirm a `404/410` from `web-push` sets `failed_at` (inspect after any stale endpoint): `sqlite3 /home/fms/data/money.sqlite "SELECT count(*) FROM push_subscriptions WHERE failed_at IS NOT NULL;"`. Expected: `0` for the just-created live subscription (it is healthy). Record the end-to-end PASS in `docs/deploy/RUNBOOK.md` and commit: `git add docs/deploy/RUNBOOK.md && git commit -m "docs(deploy): record subscribe() end-to-end push smoke-test PASS on iPhone standalone (§12.6)"`.
...
3. `sqlite3 /home/fms/data/money.sqlite "PRAGMA journal_mode; PRAGMA integrity_check;"` → `wal` / `ok`; `.tables` shows all 9 v1 tables; `recurring_items=16`, `users=1`, card `balance_cents=740076`, `payoff_baseline_cents=740076`, EF target `100000` (5.5/5.6).
4. `crontab -u money -l` → three jobs (`backup-db.sh` 03:15, `restore-verify.sh` 03:45, `run-due-watchdog.sh` `*/5`) under `TZ=Asia/Kuala_Lumpur`; `tail -1 /home/fms/backups/restore-verify.log` → `restore-verify OK`; newest `gdrive-money:money-fms-backups/` object matches the local archive (5.10/5.11/5.12).
...
**Files produced (repo-tracked):** `/Users/brendxn___/Desktop/Personal-FMS/.gitignore`, `.env.example`, `bin/deploy.sh`, `bin/backup-db.sh`, `bin/restore-verify.sh`, `bin/run-due-watchdog.sh`, `docs/deploy/RUNBOOK.md`. **Files produced (box-only, never committed):** `/home/fms/htdocs/fms.argontechs.dev/.env` (chmod 600), `/home/fms/.config/rclone/rclone.conf` (chmod 600), `/home/fms/data/money.sqlite`, `ecosystem.config.cjs` PM2 dump, the site-user crontab, the `pm2-money` systemd unit.