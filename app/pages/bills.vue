...
// ─── RecurringRow inline component ───────────────────────────────────────────
const RecurringRow = defineComponent({
  name: 'RecurringRow',
  props: {
    item: { type: Object as () => RecurringItem, required: true },
    isLast: { type: Boolean, default: false },
    deleteConfirmId: { type: Number, default: null },
    pausingId: { type: Number, default: null },
    deletingId: { type: Number, default: null },
    accountName: { type: String, default: '' },
    isCardFunded: { type: Boolean, default: false },
  },
  emits: ['edit', 'toggle-active', 'prompt-delete', 'cancel-delete', 'confirm-delete'],
  setup(props, { emit }) {
    const amountDisplay = computed(() => {
      const prefix = props.item.is_variable ? '~' : ''
      const val = 'RM' + (props.item.amount_cents / 100).toLocaleString('en-MY', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
      return prefix + val
    })
    const cadenceShort = computed(() => {
      return { monthly: 'mo', weekly: 'wk', biweekly: 'biwk', yearly: 'yr' }[props.item.cadence] ?? props.item.cadence
    })
    ...
    return { amountDisplay, cadenceShort, dueLine, emit }
  },
  template: `
    <div :class="['rrow', { 'rrow--paused': !item.is_active, 'rrow--last': isLast }]">
      <div class="rrow__main">
        <div class="rrow__left">
          <div class="rrow__icon-wrap" :class="item.direction === 'income' ? 'rrow__icon-wrap--income' : 'rrow__icon-wrap--expense'">
            <svg v-if="item.category === 'income'" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
              <polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/>
            </svg>
            <svg v-else-if="item.category === 'bills'" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
              <polyline points="1 6 1 22 23 22 23 6"/><path d="M1 6 12 1l11 5"/><path d="M8 22V12h8v10"/>
            </svg>
            <svg v-else-if="item.category === 'debt'" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
        ...
        <div class="rrow__right">
          <span :class="['rrow__amount', 'tabnum', item.direction === 'income' ? 'rrow__amount--income' : 'rrow__amount--expense']">
...
:deep(.rrow--last) { border-bottom: none; }

:deep(.rrow--paused) { opacity: 0.6; }
...
:deep(.rrow__icon-wrap--income) {
  background: rgba(5, 150, 105, 0.12);
  color: var(--positive);
}

:deep(.rrow__icon-wrap--expense) {
...
:deep(.rrow__amount--income) { color: var(--positive); }
:deep(.rrow__amount--expense) { color: var(--text); }

:deep(.rrow__paused-badge) { font-size: 11px; padding: 2px 7px; }
...
:deep(.rrow__action-btn:hover:not(:disabled)) {
  background: var(--border);
  color: var(--text);
}

:deep(.rrow__action-btn:focus-visible) {
  outline: 2px solid var(--ring);
  outline-offset: 2px;
}

:deep(.rrow__action-btn:disabled) {
  opacity: 0.5;
  cursor: not-allowed;
}

:deep(.rrow__action-btn--edit:hover:not(:disabled)) { color: var(--primary); }

:deep(.rrow__action-btn--delete) {
  margin-left: auto;
}

:deep(.rrow__action-btn--delete:hover:not(:disabled)) {
  background: rgba(220, 38, 38, 0.08);
  color: var(--negative);
  border-color: var(--negative);
}

:deep(.rrow__action-btn--delete-confirm) {
  background: rgba(220, 38, 38, 0.10);
  color: var(--negative);
  border-color: var(--negative);
}

:deep(.rrow__action-btn--delete-confirm:hover:not(:disabled)) {
